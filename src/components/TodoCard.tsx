...
const TodoCard = ({
  ...
  const [checkList, setCheckList] = useState(todo.checkList || []); // Initialize with todo.checkList
  ...
  const handleCheckboxChange = async (index: number) => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://pro-manage-backend-psi.vercel.app/api/v1/todo/update/${todo._id}`,
        {
  ...
  const handleDelete = async()=>{
    try {
      ...
      handleRemove(itemKey, 'todo'); // Update the parent component's state to remove the deleted todo
  ...
  return (
    <div className="bg-white w-[95%] auto p-4 rounded-lg shadow-md flex flex-col justify-between relative">
         {
        copied && (
          <div className=" text-center fixed top-5 right-2 bg-sky-100 shadow-lg text-black border-3 border-cyan-400 rounded-xl w-[170px] p-3 z-10">
            Link Copied!
          </div>
        )
      }
  ...
  <div className="absolute top-9 right-2 bg-white shadow-lg border border-gray-300 rounded-lg w-[150px] p-3 z-10 flex flex-col gap-3">
        ...
        {!isCollapsed && (
          <div className="w-full h-auto rounded flex flex-col m-3 mt-0">
            <div className=" flex flex-col gap-3">
              {checkList &&
                checkList.map(
                  (item: { isDone: boolean; title: string }, index: number) => (
                    <div
                      key={index}
                    ...
                    >
                      <input
                        type="checkbox"
                        className="text-black w-[1rem] h-[1rem] mt-1"
                        checked={item.isDone}
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <button
            className="bg-gray-300 text-[14px] text-gray-800 px-2 py-1 rounded"
            onClick={() => handleChange(todo?._id, "backlog", "todo", itemKey)}
          >
            Backlog
          </button>
          <button
            className="bg-gray-300 text-[14px] text-gray-800 px-2 py-1 rounded"
            onClick={() =>
              handleChange(todo?._id, "inProgress", "todo", itemKey)
            }
          >
            Progress
          </button>
          <button
            className="bg-gray-300 text-[14px] text-gray-800 px-2 py-1 rounded"
            onClick={() => handleChange(todo?._id, "done", "todo", itemKey)}
          >
            Done
          </button>
        </div>
