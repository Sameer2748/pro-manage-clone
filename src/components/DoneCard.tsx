...
const DoneCard = ({ 
  ...
  const handleDelete = async()=>{
    try {
      ...
      handleRemove(itemKey, 'done'); // Update the parent component's state to remove the deleted todo
  ...
  <div className="absolute top-9 right-2 bg-white shadow-lg border border-gray-300 rounded-lg w-[150px] p-3 z-10 flex flex-col gap-3">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <button className="bg-gray-300 text-[14px] text-gray-800 px-2 py-1 rounded" onClick={()=> handleChange(todo?._id, "backlog","done",itemKey)}>Backlog</button>
          <button className="bg-gray-300 text-[14px] text-gray-800 px-2 py-1 rounded" onClick={()=> handleChange(todo?._id, "inProgress","done",itemKey)}>Progress</button>
          <button className="bg-gray-300 text-[14px] text-gray-800 px-2 py-1 rounded" onClick={()=> handleChange(todo?._id, "todo","done",itemKey)}>Todo</button>
        </div>
      </div>
    </div>
  );
