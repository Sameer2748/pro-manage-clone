...
const handleDelete = async()=>{
  try {
    ...
    handleRemove(itemKey, 'backlog'); // Update the parent component's state to remove the deleted todo
...
return (
  <div className="bg-white w-[95%] auto p-4 rounded-lg shadow-md flex flex-col justify-between relative">
     {
        copied && (
          <div className="absolute top-0 right-0 w-[90px] h-[40px] border  border-gray-500 bg-cyan-100">
            Link Copied!
          </div>
        )
      }
  ...
  <div className="absolute top-9 right-2 bg-white shadow-lg border border-gray-300 rounded-lg w-[150px] p-3 z-10 flex flex-col gap-3">
    ...
    <div className="flex justify-between items-center">
      ...
      <div className="flex gap-2">
        <button className="bg-gray-300 text-[14px] text-gray-800 px-2 py-1 rounded" onClick={()=> handleChange(todo?._id, "todo","backlog",itemKey)}>Todo</button>
        <button className="bg-gray-300 text-[14px] text-gray-800 px-2 py-1 rounded" onClick={()=> handleChange(todo?._id, "inProgress","backlog",itemKey)}>InProgress</button>
        <button className="bg-gray-300 text-[14px] text-gray-800 px-2 py-1 rounded" onClick={()=> handleChange(todo?._id, "done","backlog",itemKey)}>Done</button>
      </div>
    </div>
  </div>
  );
